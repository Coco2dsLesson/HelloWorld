...
//	log("Setting init");
  ...
  */ 
	...
	auto soundOnMenuItem = MenuItemSprite::create(onSprite,onSprite);
	auto soundOffMenuItem = MenuItemSprite::create(offSprite,offSprite);
    ...
    this->addChild(mn);
	musicToggle = [](bool tog){return tog;};
//	musicToggle = true;
    return true;
}
...
//	log("Setting onEnterTransitionDidFinish");
	if(musicToggle){
//		log("musicToggle is %d",musicToggle);
		SimpleAudioEngine::getInstance()->playBackgroundMusic("sound/Synth.mp3", true);
	}
}
